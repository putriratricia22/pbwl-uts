...
class transaksi extends koneksi {
    ...
    public function simpan()
    {
        ...
        $transaksi_pemilik = $_POST['transaksi_pemilik'];
        $transaksi_id = $_POST['transaksi_id'];

        $sql = "INSERT INTO tb_transaksi (transaksi_pelanggan_id, transaksi_tanggal, transaksi_pemilik)
        VALUES (:transaksi_pelanggan_id, :transaksi_tanggal, :transaksi_pemilik)";
        $stmt = $this->db->prepare($sql);
        $stmt->bindParam(":transaksi_pelanggan_id", $transaksi_pelanggan_id);
        $stmt->bindParam(":transaksi_tanggal", $transaksi_tanggal);
        $stmt->bindParam(":transaksi_pemilik", $transaksi_pemilik);
        $stmt->execute();
    }
    ...
    public function update()
    {
        ...
        transaksi_pemilik=:transaksi_pemilik WHERE transaksi_id=:transaksi_id";
        $stmt = $this->db->prepare($sql);
        $stmt->bindParam(":transaksi_pelanggan_id", $transaksi_pelanggan_id);
        $stmt->bindParam(":transaksi_tanggal", $transaksi_tanggal);
        $stmt->bindParam(":transaksi_pemilik", $transaksi_pemilik);
